...
public:
  JoystickControllerNode() : Node("joystick_controller_node") {
    ...
    sub_left_x_ = this->create_subscription<std_msgs::msg::Float32>(
        "joystick/left_x", 10,
        [this](const std_msgs::msg::Float32::SharedPtr msg) { left_x_ = apply_deadzone(msg->data); }
    );

    sub_left_y_ = this->create_subscription<std_msgs::msg::Float32>(
        "joystick/left_y", 10,
        [this](const std_msgs::msg::Float32::SharedPtr msg) { left_y_ = apply_deadzone(msg->data); }
    );

    sub_third_axis_ = this->create_subscription<std_msgs::msg::Float32>(
        "joystick/third_axis", 10,
        [this](const std_msgs::msg::Float32::SharedPtr msg) { third_axis_ = apply_deadzone(msg->data); }
    );
    ...
    timer_ = this->create_wall_timer(
        std::chrono::milliseconds(100), [this]() { this->update_wheel_speeds(); });
  }
...
private:
  void update_wheel_speeds() {
    ...
    double u2 = SPEED_GAIN * (-WHEEL_BASE * theta_dot - x_dot / 2 - y_dot * SIN_PI_3) / WHEEL_RADIUS;
    double u3 = SPEED_GAIN * (-WHEEL_BASE * theta_dot - x_dot / 2 + y_dot * SIN_PI_3) / WHEEL_RADIUS;
  ...
  rclcpp::Subscription<std_msgs::msg::Float32>::SharedPtr sub_left_x_, sub_left_y_, sub_third_axis_, sub_lt_, sub_rt_;
